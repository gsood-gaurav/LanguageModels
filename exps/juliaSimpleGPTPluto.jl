...
# ╔═╡ 62daad07-3ea4-4f28-839d-30620ada9ae7
begin
	using Lux
	using Random
	using ADTypes, MLUtils, Optimisers, Zygote, OneHotArrays, Random, Statistics, Printf
end

# ╔═╡ f1811223-518f-497b-861c-05440c8ffb42
md"""
# Implementation GPT in Julia
"""

# ╔═╡ 69b02ddc-808d-11ef-3eee-a5c13ddec0cc
begin
	text = open(io->read(io, String), "../input.txt")
	println("Length of dataset in characters  $(length(text))")
	# Let's look at first 1000 charachters
	println(@views text[1:1000])
end

# ╔═╡ 6ac3acb3-7918-4e8a-b82d-fd043bb6e70a
begin
	# here are all unique character that occur in this text
...
# ╔═╡ 98a49e62-41d8-43d5-97a3-f850c83371b6
md"""
# Tokenization
Either we can have long sequence of integers and short vocab size or short sequence of integers with long vocab size. For example GPT2 with a vocab size of 50000 will produce shorter sequence of ints as compared to same sequence encoded with charachter level tokenizer with 65 vocan size
"""
...
begin
	...
	itos = Dict{Int, Char}([(v, k) for (k, v) in pairs(stoi)])
	# Encode the given sentence into sequence of ints (tokens)
	# This is idea behind tokenization in Large Lanugage Models
	encode(s::String) = [stoi[token] for token in s]
	# Decode given sequence of ints to string/text
	# it is opposite of encode
	decode(is::Vector{Int}) = join([itos[i] for i in is])
	@assert "how are you" === decode(encode("how are you"))
	# encode("hii there")
end

# ╔═╡ e0a82ea3-37a7-4e82-aaad-fdfb4cf9ed6c
begin
...
begin
	# Let's now split our data set into train and validation split
	n = floor(Int, length(data) * 0.9)
	train_data = @views data[1:n]
	val_data = @views data[n+1:end]
end

# ╔═╡ d5be23e9-62d6-43e0-b1d0-7e99ffbe498e
md"""
# Note on Context Length
When we feed samples to transformers we feed samples starting from as little as 1 and max at block_size. This helps during inference such that we start generation process using even a single token. It can generate till block_size and after that we should start truncating because transformer will never see input more than block_size. But this issue is circumvented in latest transformer which has infinite context lengths.
"""
...
begin
	...
	function get_batch(split)
		...
		ix = rand(MersenneTwister(1337), 1:(length(data)-block_size), (batch_size,))
	...
	println("Inputs:")
	println(size(xb))
	println(permutedims(xb, (2, 1)))
	println(size(yb))
	println(permutedims(yb, (2, 1)))
	# Independent 32 Examples
	for b in 1:batch_size
		for t in 1:block_size
			context = xb[1:t, b]
			target = yb[t, b]
			println("When context is $(context) target: $(target)")
		end
	end
...
begin
	...
	ps, state = Lux.setup(Lux.replicate(rng), model)
	lossfn = CrossEntropyLoss()
	logits, state = model(xb, ps, state)
	lossfn(logits, onehotbatch(yb, 1:vocab_size))
end

# ╔═╡ 1ef15240-42ce-4cec-b095-b74274a7ef45
onehotbatch(yb, 1:65)
...
manifest_format = "2.0"
project_hash = "8d8ad744a0886d6da7e7baaf23990dae6afe8328"
...
git-tree-sha1 = "539ed78ba952e68a7751941e7b8fbae7f15ba70f"
